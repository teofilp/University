...
public class RunProgramPage {
    ...
    @FXML
    private TableView<SemaphoreTableRow> semaphoreTV;
    ...
    private void updateLayout(IList<ProgramState> programStates) {
        ...
        updateSemaphoreTableView(state);
        updateActiveThreadId();
    }

    private void updateSemaphoreTableView(ProgramState state) {
        var st = state.getSemaphoreTable();
        semaphoreTV.getItems().setAll(state.getSemaphoreTable()
                .getKeys()
                .stream()
                .map(x -> new SemaphoreTableRow(x.toString(), st.get(x).getKey().toString(), st.get(x).getValue()))
    ...
    private void setupLayout() {
        ...
        setupSemaphoreTableView();
        setupRunOnceButton();
    }

    private void setupSemaphoreTableView() {
        var idCol = new TableColumn("Descriptor");
        var countCol = new TableColumn("Count");
        var valueCol = new TableColumn("Values");

        idCol.setCellValueFactory(new PropertyValueFactory<>("key"));
        valueCol.setCellValueFactory(new PropertyValueFactory<>("value"));
        countCol.setCellValueFactory(new PropertyValueFactory<>("count"));

        semaphoreTV.getColumns().setAll(idCol, countCol, valueCol);
        semaphoreTV.setMaxHeight(200);
    }
