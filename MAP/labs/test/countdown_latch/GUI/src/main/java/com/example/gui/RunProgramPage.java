...
public class RunProgramPage {
    ...
    @FXML
    private TableView<LatchTableRow> latchTV;
    ...
    private void updateLayout(IList<ProgramState> programStates) {
        ...
        updateLatchTableView(state);
        updateActiveThreadId();
    }

    private void updateLatchTableView(ProgramState state) {
        latchTV.getItems().setAll(
          state.getLatchTable().getKeys()
                  .stream()
                  .map(x -> new LatchTableRow(x.toString(), String.valueOf(state.getLatchTable().get(x))))
    ...
    private void setupLatchTableView() {
        var descriptorCol = new TableColumn("Descriptor");
        var valueCol = new TableColumn("Count");

        descriptorCol.setCellValueFactory(new PropertyValueFactory<>("key"));
        valueCol.setCellValueFactory(new PropertyValueFactory<>("value"));

        latchTV.getColumns().setAll(descriptorCol, valueCol);
        latchTV.setMaxHeight(200);
    }
