...
public class RunProgramPage {
    ...
    @FXML
    private TableView<BarrierTableRow> barrierTV;
    ...
    private void updateLayout(IList<ProgramState> programStates) {
        ...
        updateBarrierTableView(state);
    }

    private void updateBarrierTableView(ProgramState state) {
        var barrierTable = state.getBarrierTable();

        barrierTV.getItems().setAll(barrierTable.getKeys().stream().map(x -> {
            var value = barrierTable.get(x);
            var values = value.getValue().stream().map(Object::toString).reduce("", (acc, curr) -> acc + ", " + curr);
            return new BarrierTableRow(x, value.getKey(), values);
    ...
    private void setupLayout() {
        ...
        setupBarrierTableView();
        setupRunOnceButton();
    }

    private void setupBarrierTableView() {
        var descriptorCol = new TableColumn("Descriptor");
        var sizeCol = new TableColumn("Size");
        var valuesCol = new TableColumn("values");

        descriptorCol.setCellValueFactory(new PropertyValueFactory<>("index"));
        sizeCol.setCellValueFactory(new PropertyValueFactory<>("value"));
        valuesCol.setCellValueFactory(new PropertyValueFactory<>("waitingThreads"));

        barrierTV.getColumns().setAll(descriptorCol, sizeCol, valuesCol);
        barrierTV.setMaxHeight(200);
    }
